...
1. Folder Structure and Key Files (map)
1. Data Contract and Persistence (ORM + schema)
1. Service-by-Service Deep Dive (APIs, internals)
1. Local Setup, Running, and Scripts
1. Developer Tasks and Workflows
1. Theory and Design Rationale
1. Troubleshooting and Diagnostics
1. Roadmap and Next Steps
1. Contribution Guide
1. Appendix: Quick Commands and References
...
1. Detection processes text and returns label + confidence (stateless except for writes).
1. Risk analytics aggregates and stores records for dashboard/queries.
...
- services/
  ...
  - detection/
    - main.py: FastAPI app; health `/`, detect `/detect`, list models `/models`, active detector `/detector`.
    - detector.py: DetectorRegistry and base interfaces.
    - detectors/
      - simple_detector.py: Lightweight heuristic baseline detector.
...
- Endpoints:
  ...
  - GET /detector → active detector + available detectors
  - POST /detector/{name} → switch active detector (`simple`, `rag`, `zero-shot`)

- Internals:
  - DetectorRegistry returns the configured/default detector (set via `DETECTOR_NAME` or switched at runtime)
  - get_detector() lazily initializes to reduce cold-start latency elsewhere
  - Detector.detect() returns label + confidence + timestamp

Operational notes:

- `DETECTOR_NAME` controls the start-up detector selection.
- `ZERO_SHOT_MODEL_PATH` can point to a pre-downloaded Hugging Face model directory for fully offline zero-shot inference.
- `RAG_MODEL_PATH` can point to a pre-downloaded Sentence Transformer directory for fully offline RAG embedding inference.
- Helper scripts:
...
- Endpoints:
  ...
  - POST /dashboard/analyze-text → analyze ad-hoc text from the UI
  - POST /dashboard/upload-file → analyze uploaded text files from the UI
  - POST /dashboard/set-detector → switch detection model from the UI
...
- Use [http://127.0.0.1:PORT](http://127.0.0.1:PORT) instead of localhost.
